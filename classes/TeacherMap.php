...
class TeacherMap extends BaseMap {
  ...
  public function findAll($ofset = 0, $limit = 20) {
    ...
    . " INNER JOIN role ON user.role_id=role.role_id WHERE role.sys_name='teacher' LIMIT $ofset, $limit");
  ...
  public function findProfileById($id) {
    if ($id) {$res = $this->db->query("SELECT teacher.user_id, otdel.name AS otdel FROM teacher "
