...
class UserMap extends BaseMap {
  ...
  public function arrRoles() {
    $res = $this->db->query("SELECT role_id AS id, sys_name, name AS value FROM role");
    return $res->fetchAll(PDO::FETCH_ASSOC);
  }
  ...
  public function findAllByRole($role_id, $ofset = 0, $limit = 30) {
    if($role_id) {
      $res = $this->db->query("SELECT user.user_id, CONCAT(user.lastname,' ', user.firstname, ' ', user.patronymic) AS fio, user.birthday,"
      . " gender.name AS gender, role.name AS role FROM user"
      . " INNER JOIN gender ON user.gender_id=gender.gender_id"
      . " INNER JOIN role ON user.role_id=role.role_id WHERE role.role_id=$role_id LIMIT $ofset, $limit");
